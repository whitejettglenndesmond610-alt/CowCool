...
<template>
  <div ref="rootRef" class="pointer-events-none invisible fixed inset-0 z-[70] overflow-hidden" :style="rootStyle" aria-hidden="true">
    ...
    <div class="absolute inset-0 z-10 opacity-30" style="background-image: radial-gradient(circle, rgba(21,32,29,.18) 1px, transparent 1px); background-size: 28px 28px; mask-image: radial-gradient(circle at center, #000, transparent 70%)" />
    <span class="absolute -bottom-[12%] left-[3%] z-10 -rotate-6 text-[38vw] font-extrabold leading-none tracking-[-0.12em] text-[var(--transition-ink)] opacity-[0.035]">{{ toPage.number }}</span>
    <span class="absolute right-[4%] top-[13%] z-10 rotate-6 text-[15vw] font-extrabold leading-none tracking-[-0.08em] text-[var(--transition-ink)] opacity-[0.035]">{{ toPage.label }}</span>
    <div class="absolute left-[6%] top-[12%] z-10 h-36 w-56 -rotate-12 opacity-40" :style="{ backgroundColor: palette.mid, clipPath: 'polygon(8% 8%, 100% 0, 86% 89%, 17% 100%, 0 55%)' }" />
    <div class="absolute bottom-[7%] right-[5%] z-10 h-40 w-64 rotate-12 opacity-35" :style="{ backgroundColor: palette.back, clipPath: 'polygon(17% 0, 100% 24%, 79% 100%, 0 73%)' }" />
    <svg class="absolute inset-0 z-10 h-full w-full opacity-35" viewBox="0 0 1200 800" preserveAspectRatio="none" fill="none" aria-hidden="true">
