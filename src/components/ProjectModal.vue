...
<template>
  <Teleport to="body">
    ...
    >
      ...
      <div class="modal-panel relative w-full max-w-[560px] h-screen bg-[#0c0e14] border-l border-white/5 md:border-l px-10 py-16 md:px-10 md:py-16 overflow-y-auto md:max-w-[560px] md:h-screen md:rounded-none
                  max-md:max-w-full max-md:h-[90vh] max-md:border-l-0 max-md:border-t max-md:rounded-t-2xl max-md:mt-auto max-md:bottom-0"
      >
        <Button
          variant="ghost"
          size="icon"
          class="absolute top-5 right-5 rounded-full text-white/60 hover:text-white hover:bg-white/5 modal-close-btn"
          @click="handleClose"
        >
          <X class="size-5" />
        </Button>
        <div class="modal-body" v-if="data">
          <span class="modal-fade-in block text-xs font-semibold uppercase tracking-[0.1em] text-[#00c8e8] mb-2 font-mono">
            {{ data.type }}
          </span>
          <h2 class="modal-fade-in text-3xl font-extrabold text-white mb-5" style="font-family: Inter, sans-serif;">
            {{ data.title }}
          </h2>
          <p class="modal-fade-in text-white/70 leading-relaxed mb-7">
            {{ data.desc }}
          </p>
          <div class="modal-fade-in flex flex-wrap gap-2 mb-7">
            <span
              ...
              style="background: rgba(108,92,231,0.1); border-color: rgba(108,92,231,0.25); color: #7c3aed;"
            >
              {{ t }}
            </span>
          </div>
          <ul class="space-y-3">
            <li v-for="h in data.highlights" :key="h" class="modal-fade-in relative pl-5 text-white/70 text-sm leading-relaxed">
              <span class="absolute left-0 top-2 w-2 h-2 rounded-full bg-[#00c8e8]" style="box-shadow: 0 0 8px #00c8e8;" />
              {{ h }}
            </li>
          </ul>
          ...
          <div v-if="data.demo || data.github" class="modal-fade-in flex gap-3 mt-8">
            <a v-if="data.demo" :href="data.demo" target="_blank" rel="noopener"
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#00c8e8] to-[#7c3aed] text-white text-sm font-semibold hover:shadow-[0_0_30px_rgba(108,92,231,0.3)] transition-shadow">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
              在线演示
            </a>
            <a v-if="data.github" :href="data.github" target="_blank" rel="noopener"
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/20 text-white text-sm font-semibold hover:border-[#7c3aed] hover:text-[#7c3aed] transition-colors">
